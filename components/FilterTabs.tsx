...
export default function FilterTabs({
  ...
  return (
    <div
      className="flex items-center gap-2 overflow-x-auto scrollbar-none flex-nowrap sm:flex-wrap pb-1 sm:pb-0 max-w-full touch-pan-x"
      role="tablist"
      aria-label="Market view filters"
    >
      {tabs.map((tab) => {
        ...
        const buttonContent = (
          <>
            ...
            {tab.count !== undefined && (
              <span
                className={`text-[11px] font-semibold px-1.5 py-0.5 rounded-full tabular-nums transition-colors shrink-0 ${
                  isActive
        ...
        const className = `h-9 px-3.5 rounded-lg text-xs md:text-sm font-medium transition-all flex items-center gap-2 cursor-pointer select-none group shrink-0 focus:outline-none focus:ring-1 focus:ring-[#FF5446]/40 ${
          isActive
